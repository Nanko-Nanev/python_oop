class PizzaDelivery:
    ...
    def make_order(self):
        self.ordered = True
        current_ingredients = [f"{i}: {q}" for i, q in self.ingredients.items()]
        return f"You've ordered pizza {self.name} prepared with {', '.join(current_ingredients)} and the price will be {self.price}lv."
